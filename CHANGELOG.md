...
- **[feature] Public headless sync.** Host apps can now trigger a full sync of the user's primary data source (Rolla band, Apple Health, or Health Connect) without presenting any SDK UI, then read the result. The result is always a well-typed terminal outcome — `success` (with whether any new data was uploaded and the latest server sync time), `skipped` (with a documented reason: no band connected, a sync already running, a server-side source like Garmin/Oura, offline, or not initialized), or `failure` — it never throws. A completion callback (iOS) / listener (Android) reports the same result. This is foreground-only in this release; true deep-background sync is a planned follow-up.
