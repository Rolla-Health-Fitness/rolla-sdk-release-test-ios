...
- **[feature] Added Leaderboards.** Opt-in competitive rankings that compare users on their Health Score or Active Points against others in the tenant, over weekly and monthly periods. The profile screen shows a summary card with the user's rank per challenge type; the detail page lists ranked participants centered on the user's position with bidirectional pagination, supports weekly/monthly toggling and browsing up to 6 months of history, and lets users join or leave (with a 7-day rejoin cooldown). Wired end-to-end to the backend leaderboard API.
- **[fix] Band pairing no longer flickers before the firmware verdict.** After pairing, the firmware step briefly flashed "Firmware up to date", then "Checking firmware…", then "up to date" again while the band's version was still being read. The step now holds a single "Checking firmware…" state until the check produces a real verdict — including when the version can't be read at all, which previously rendered a false "up to date" without checking.
- **[feature] Leaderboards can be disabled.** `RollaDisabledModule` gains a `leaderboards` value: pass it in the `disabledModules` configuration (alongside the existing `weight` and `bloodPressure`) to hide the Leaderboards module everywhere it appears in the SDK UI.
- **[fix] Leaderboards are reachable when the bottom navigation is hidden.** Hosts using `hideBottomNavigation` never see the Profile screen, which was the only entry point to Leaderboards. A Leaderboards shortcut now appears in the Home settings sheet, opening the same challenge cards and join flow. It is hidden when the Leaderboards module is disabled.
- **[improvement] Onboarding can be skipped by setting the profile in advance.** Host apps can now call `POST /api/setprofile` (with the user's bearer token and `Partner-ID` header, like the other auth-API endpoints) before first presenting the SDK: when the profile already carries a username, birthdate, gender and height — plus weight when the weight module is enabled — the SDK skips its account-details onboarding screen entirely. The completeness rule no longer demands units, country, or language (they default or self-heal), and a partially set profile pre-fills the onboarding form so users only complete the gaps. Skipping the profile form still runs the full downstream tail — consent, goals, and connecting a data source — exactly as filling the form does, so a user never reaches Home without a data source connected. The onboarding goals step now also shows a back button for host integrations that set `showBackButton`, so a user is never stuck there without a way back to the host app.
- **[feature] Host-controlled SDK language.** `RollaConfiguration` gains an optional `language`, typed by the new `RollaLanguage` enum listing every language the SDK ships (`english`, `german`, `spanish`, `croatian`, `bosnian`, `serbianLatin`, `serbianCyrillic`, `arabic`). When set, that language is authoritative for the Flutter engine's lifetime: persisted picks and the backend profile cannot override it. Changing the configured language requires destroying and recreating the engine, like other SDK configuration changes. When `language` is not set, the existing profile-driven behavior (with the branding `defaultLocale` as pre-profile fallback) stays as-is. A configured language is also written to the user's backend profile at startup when it differs, so backend-generated content (goal labels, insights) matches the SDK UI language.
- **[fix] Language now remains selected after logout.** The SDK no longer treats locale as per-user cache or removes it during logout storage cleanup, allowing profile-driven Flutter hosts to keep their auth UI in the last selected language. The next backend profile or configured host language can still overwrite it.
- **[fix] "Health Score" stays the brand term in Serbian.** The Serbian translations (both Latin and Cyrillic) were rendering "Health Score" as a translated phrase; it now keeps the "Health Score" brand mark verbatim, matching every other language including Croatian and Bosnian.
- **[feature] Host event listener: seven new SDK events pushed to the host app.** The existing `RollaDelegate` (iOS) / `RollaListener` (Android) gains seven methods a host can override to observe what happens inside the SDK, without polling:
  ...
  All methods have default no-op bodies, so existing integrations compile unchanged. Events are delivered for the engine's lifetime — they keep flowing after the SDK UI closes, as long as the engine is alive.
- **[feature] Host event listener: four more SDK events — band link and activity lifecycle.** `RollaDelegate` (iOS) / `RollaListener` (Android) gains four more methods, riding the same engine-lifetime delivery as the seven above:
  ...
  All methods have default no-op bodies, so existing integrations compile unchanged. Firing semantics and lifecycle guarantees are documented on the delegate/listener methods.
