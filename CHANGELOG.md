...
- **[feature] Open a specific SDK screen from the host app.** The new `openScreen` API (Flutter: `RollaSDK.openScreen`, iOS/Android wrappers: `openScreen`) opens the Insights feed, the activity history, or the goals editor directly — presenting the SDK UI first when it is not on screen, honoring an optional presentation transition. The opened screen is the root of the SDK UI: back returns the user straight to your app. Every outcome is a typed `RollaOpenScreenStatus`; the SDK's mandatory startup steps (onboarding, consent, permissions, data-source connection) always take precedence over the request.
