...
- **[feature] Host event listener: seven new SDK events pushed to the host app.** The existing `RollaDelegate` (iOS) / `RollaListener` (Android) gains seven methods a host can override to observe what happens inside the SDK — no polling and no need to wait for the SDK UI to close:
  - **Activity completed** (`rollaDidCompleteActivity` / `onActivityCompleted`) — fires per lifecycle phase: `finished` the moment the activity is saved in-SDK, then `uploaded` once the backend confirms it, or `uploadFailed` if the upload permanently gives up. The payload carries the activity summary (type, duration, distance, calories, times) and a `source` field distinguishing live-tracked activities from manually logged ones. Key idempotency on `(activityId, phase)` — the terminal phases can re-fire across retries.
  - **UI sync completed** (`rollaDidCompleteUISync` / `onUiSyncCompleted`) — a sync finished inside the SDK UI (auto-sync on open, return from background, in-app manual refresh). Carries the same result type as `syncHealthData`, so hosts read one shape for both directions; the headless `syncHealthData` keeps its own separate callback.
  - **Band paired / band unpaired** (`rollaDidPairBand`+`rollaDidUnpairBand` / `onBandPaired`+`onBandUnpaired`) — the user paired or unpaired a band inside the SDK UI. Auto-reconnects, login restores, and remote unpairs from other devices do not fire these.
  - **Primary source changed** (`rollaDidChangePrimarySource` / `onPrimarySourceChanged`) — the SDK observed the user's primary data source change, with previous and current source.
  - **Goals changed** (`rollaDidChangeGoals` / `onGoalsChanged`) — the user saved goal changes (each toggle backend-confirmed). One event per save, carrying both the toggled goals and the resulting enabled set.
  - **Profile updated** (`rollaDidUpdateProfile` / `onProfileUpdated`) — the user edited profile details or logged weight / blood pressure. The payload is a sparse map of only the changed fields (e.g. `{weightKg: 98.0}`) with a documented key vocabulary — hosts inspect the keys they care about and ignore the rest.

  All new methods have default no-op bodies, so existing integrations compile unchanged. Events are delivered for the engine's lifetime: they keep flowing after the SDK UI closes and work fully headlessly (a host that only ever calls the headless APIs still receives them).
- **[improvement] Activity uploads that keep failing now go terminal instead of retrying forever.** Failed uploads retry on later in-SDK syncs as before, but after 5 failed attempts — or immediately on a non-retryable backend rejection — the SDK stops retrying and reports the `uploadFailed` phase to the host event listener.
- **[improvement] The in-memory primary-source cache resets on logout/user switch.** Prevents one account's primary source, sync recency, and sync checkpoints from bleeding into the next account on a shared device.
...
- **[feature] New headless public SDK methods.** Three new methods that all run **headlessly** — no SDK UI needs to be opened; after initializing the SDK, host apps can call them directly:
  - **`warmUpEngine()`** — starts and configures the SDK ahead of time without showing any UI, so the first `show()` is instant and the headless reads below work before the SDK has ever been presented. Optional: the two methods below already warm up the engine if needed, but this method gives host apps the freedom to warm the engine separately and control the timing.
  - **`syncHealthData()`** — runs a full sync of the user's primary health data source and returns a typed result: `success` (with whether new data was uploaded, when it completed, and a `syncedData` breakdown of what was synced), `skipped` (with a documented reason, e.g. no band connected or a missing permission), or `failure`. It never throws, and reports the same result to a completion callback (iOS) / listener (Android). Pass `includeSamples: true` to also get the raw per-sample arrays.
  - **`getBandBatteryLevel()`** — a live battery read from the connected Rolla band, returning a typed result: a percentage when available, or a documented "unavailable" reason (no band paired, disconnected, timed out, Bluetooth off, permission not granted).
