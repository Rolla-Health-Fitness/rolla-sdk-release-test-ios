...
- **[feature] New headless public SDK methods.** Four new methods that all run **headlessly** — no SDK UI needs to be opened; after initializing the SDK, host apps can call them directly:
  ...
  - **`syncHealthData()`** — runs a full sync of the user's primary health data source and returns a typed result: `success` (with whether new data was uploaded, when it started and completed, and a `syncedData` breakdown of what was synced), `skipped` (with a documented reason, e.g. no band connected or a missing permission), or `failure`. It never throws, and reports the same result to the `rollaDidCompleteHealthDataSync` delegate method (iOS) / `onSyncHealthDataCompleted` listener method (Android) — named 1:1 after the method so it can't be confused with the UI-sync event. Pass `includeSamples: true` to also get the raw per-sample arrays.
...
- **[feature] Host event listener: four more SDK events — band link and activity lifecycle.** `RollaDelegate` (iOS) / `RollaListener` (Android) gains four more methods, riding the same engine-lifetime delivery as the seven above:
  ...
  Band connected/disconnected report the band's **live BLE link** transitions (the pairing state is answered by `getPairedBandInfo()`): connect fires immediately, disconnect is debounced a few seconds to filter flap noise and arrives after the BLE supervision timeout — these are not a proximity signal, and nothing fires while the engine is cold. Activity started fires when a live tracking session begins — `origin` distinguishes a fresh start from a crash-recovery resume (re-fired so hosts can re-sync; dedupe on `activityId`); manual activities enter the lifecycle at `finished` and never fire it. Activity removed fires when a record is gone without a kept result — `reason` `canceled` (a crash-recovery discard; the commonly expected "activity canceled" case) or `deleted` (the user deleted a saved activity, backend-confirmed). Together these close the activity lifecycle: every started activity ends in `finished` or `removed` — with one exception: sessions abandoned mid-tracking for over a day are cleaned up silently, without an event. All methods have default no-op bodies, so existing integrations compile unchanged.
