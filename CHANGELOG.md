...
- **[feature] Hide selected data sources from the SDK UI.** A new `disabledDataSources` configuration option lets host apps hide specific data-source connect options (Band, Garmin, Oura, Apple Health, Health Connect) everywhere the user picks a source to connect — the Data Sources page and the onboarding data-source gate. Omit it or pass an empty set to offer every source (default, no change for existing partners). A source a user has already connected stays visible so it can be viewed or disconnected. When the Rolla Band is the only remaining source, the data-source picker is skipped entirely: onboarding routes straight to band pairing (required before the app opens), and the Settings "Data Sources" entry is hidden since there is nothing to choose — band status and unpairing remain available from the band button on the Home screen.
