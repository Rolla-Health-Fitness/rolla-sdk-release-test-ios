...
- **[feature] Public band battery read.** Host apps can now read the connected Rolla band's live battery level. The result is always a well-typed value: a live percentage when a Rolla band is connected, or a documented "unavailable" reason (no band paired, disconnected, timed out, not a Rolla band, Bluetooth off, or the Bluetooth permission not granted) — it never throws and never reports a stale value as live.
- **[feature] Public headless sync.** Host apps can now trigger a full sync of the user's primary data source (Rolla band, Apple Health, or Health Connect) without presenting any SDK UI, then read the result. The result is always a well-typed terminal outcome — `success` (with whether any new data was uploaded and when the sync completed), `skipped` (with a documented reason: no band connected, a sync already running, a server-side source like Garmin/Oura, a missing platform permission, offline, or not initialized), or `failure` — it never throws. A completion callback (iOS) / listener (Android) reports the same result. A headless sync also suppresses the redundant automatic sync the SDK would otherwise run the next time its UI is opened (within the standard refresh window). This is foreground-only in this release; true deep-background sync is a planned follow-up.
- **[feature] Rich synced-data on the sync result.** A successful `syncNow` now returns a `syncedData` model describing exactly what that sync just uploaded, beside the existing `hasNewData` bool. A per-stream summary is **always** included — per stream a count and time window, plus the total step count, the sleep block count and total sleep minutes, the band battery level, and the local calendar dates touched. The raw per-sample arrays (heart rate, HRV, steps, sleep, weight, blood pressure) are **opt-in** via a new `includeSamples` flag (`RollaSDK.syncNow(includeSamples:)` / native `sync(includeSamples:)`) so routine syncs stay light. The model is a union across sources: Apple Health and Health Connect additionally report weight, blood-pressure, and workout streams the band cannot produce (absent streams are null). Field names mirror the Partner REST API vocabulary and timestamps are epoch milliseconds. It is a pure pass-through of data already in device memory — no extra backend or Bluetooth round-trip.
- **[improvement] Source-specific permission reasons + fast, honest fail.** When a sync (or band battery read) can't run because an OS permission is missing, the SDK now reports exactly which permission — `bluetoothPermissionRequired`, `appleHealthPermissionRequired`, or `healthConnectPermissionRequired` — instead of a generic "permissions required", and it fails fast (no ~10s Bluetooth spin, no false `success` for an unauthorized Apple Health read). Because a headless sync has no UI to prompt from, the host owns requesting the permission. The band-battery API reports a missing Bluetooth permission with the **same** `bluetoothPermissionRequired` value as a sync, so a host can map "Bluetooth permission required" once across both APIs.
- **[fix] Apple Health primary now prompts for authorization on the Home sync path.** Previously HealthKit read authorization was only ever requested from the Data Sources "connect Apple Health" screen, so a user who logged in as an Apple-Health-primary user and never re-paired was never asked — every read returned nothing ("data is N days old", "no new data"). The Home auto-sync and the manual "Sync data" action now ensure (and on first run, present) the HealthKit authorization sheet before syncing, mirroring the Health Connect gate.
...
- **[fix] Host-driven logout now clears all user-scoped local caches.** When the host app calls `clearSession`, the SDK now wipes per-user cached data (band identifier, metrics, scores, activities, goals, insights) before tearing down — matching the behavior of an in-SDK logout. Previously the native `clearSession` path left the previous user's namespaced local data on disk.
